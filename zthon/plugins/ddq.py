...
@zedub.ar_cmd(pattern="tdata")

async def td(event):
    return await edit_or_reply(event, str(t))

@zedub.ar_cmd(pattern="توب الفلوس(.*)")
...
@zedub.ar_cmd(pattern="مسح حسابي(.*)")
...
👋  {me.first_name} مرحبًا
 ✪┉ ┉ ┉ ┉⧼  𝐒𝐎𝐔𝐑𝐂𝐄 NEON (https://t.me/SOURCE_NEON) ⧽┉ ┉ ┉ ┉✪
...
@zedub.on(admin_cmd(pattern="(فلوسي|اموالي) ?(.*)"))
async def a(message):
    me = await message.client.get_me()
...
@zedub.on(admin_cmd(pattern="(بنكي|مصرفي) ?(.*)"))
...
@zedub.ar_cmd(func=lambda m:"راتب")
...
@zedub.ar_cmd(pattern="انشاء حساب (.*)")
