...
@zedub.zed_cmd(
    ...
    info={
        ...
        "الوصـف": "اوامـر ذكيـه لـ قفـل / فتـح حمـاية المجمـوعـات بالمسـح والطـرد والتقييـد لـ اول مـره فقـط على سـورس نيون",
        "الاوامـر": {
...
@zedub.zed_cmd(
    ...
    info={
        "header": "لـ عـرض اعـدادات حمـاية المجمـوعـة الخـاصـه ببـوت نيون",
...
async def zelzal(zed):
    ...
    if con != "طرد":
        ...
        async for user in zed.client.iter_participants(zed.chat_id):
            if user.bot:
                ...
                await sleep(0.5)
        if del_u > 0:
            del_status = f"🛂**┊كشـف البـوتات -** 𝐒𝐎𝐔𝐑𝐂𝐄 𝐍𝐄𝐎𝐍\
    ...
    if del_a > 0:
        del_status = f"❇️**┊طـرد البـوتات -** 𝐒𝐎𝐔𝐑𝐂𝐄 𝐍𝐄𝐎𝐍\
