...
async def amireallyalive(event):
    ...
    start = datetime.now()
    zedevent = await edit_or_reply(event, "**⛥ ⤻ انتـظࢪ جـاࢪي فـحص بـ ـوت NEON الخـاص بـِك   ۦ**")
    end = datetime.now()
    ms = (end - start).microseconds / 1000
    _, check_sgnirts = check_data_base_heal_th()
    Z_EMOJI = gvarstatus("ALIVE_EMOJI") or "❈┊"
    ALIVE_TEXT = gvarstatus("ALIVE_TEXT") or "** سُـورس NEON يـَعمل بنـَجاح **"
...
**{Z_EMOJI} ☆ 𝚂OᑌᖇᑕE ᑕᕼᗩᑎᑎEᒪ NEON ☆  :** [اضغـط هنـا](https://t.me/SOURCE_NEON)"""
...
async def amireallyialive(event):
    ...
    reply_to_id = await reply_id(event)
    Z_EMOJI = gvarstatus("ALIVE_EMOJI") or "❈┊"
    zed_caption = "** بسُـورس NEON يـَعمل بنـَجاح **\n
